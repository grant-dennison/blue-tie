...
test("overall diagnostics", async () => {
  ...
  const workersCallCounts = diagnosticsAfterCalls.workersDiagnostics
    .map((d) => d.callCount)
    .sort()
  const workersCallCountsAdjusted = workersCallCounts.map(
    (c) => c - workersCallCounts[0]
  )
  ...
  const workersCallCounts2 = diagnosticsAfterClose.workersDiagnostics
    .map((d) => d.callCount)
    .sort()
  const workersCallCounts2Adjusted = workersCallCounts2.map(
    (c) => c - workersCallCounts2[0]
  )
